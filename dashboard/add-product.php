    ...
    if (empty($name)) {
        ...
        if (empty($error)) {
            ...
            try {
                ...
                // Clear post parameters to reset form
                $_POST = [];
            } catch (Exception $e) {
                $pdo->rollBack();
...
<body class="bg-zinc-950 text-zinc-100 min-h-screen flex">
    ...
    <main class="flex-grow flex flex-col min-h-screen overflow-y-auto">
        ...
        <section class="p-8 max-w-5xl">
            ...
            <form action="add-product.php" method="POST" enctype="multipart/form-data" class="space-y-8">
                <!-- Info Section -->
                <div class="glass-card rounded-2xl p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" required oninput="generateSlug(this.value)" value="<?php echo htmlspecialchars($_POST['name'] ?? ''); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="e.g. Apple iPhone 15 Pro Max">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-zinc-400">URL Slug (Auto-generated)</label>
                            <input type="text" id="slugInput" name="slug" value="<?php echo htmlspecialchars($_POST['slug'] ?? ''); ?>" class="w-full bg-zinc-900 border border-zinc-850 rounded-lg py-2.5 px-4 text-xs font-medium text-zinc-400 focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="e.g. apple-iphone-15-pro-max">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="sku" value="<?php echo htmlspecialchars($_POST['sku'] ?? ''); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="e.g. AP-IP15PM-256G">
                        </div>
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="brand_id" required class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-bold uppercase tracking-widest text-zinc-300 focus:outline-none focus:border-zinc-500 focus:ring-0 cursor-pointer">
                                ...
                                <?php foreach ($brands as $brand): ?>
                                    <option value="<?php echo $brand['id']; ?>" <?php echo (($_POST['brand_id'] ?? '') == $brand['id']) ? 'selected' : ''; ?>>
                ...
                <div class="glass-card rounded-2xl p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" required name="price" value="<?php echo htmlspecialchars($_POST['price'] ?? ''); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="0.00">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-zinc-400">Compare At Price ($ USD)</label>
                            <input type="number" step="0.01" name="compare_at_price" value="<?php echo htmlspecialchars($_POST['compare_at_price'] ?? ''); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="0.00">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="number" required name="stock_quantity" value="<?php echo htmlspecialchars($_POST['stock_quantity'] ?? '10'); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="0">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-zinc-400">Low Stock Limit *</label>
                            <input type="number" required name="low_stock_limit" value="<?php echo htmlspecialchars($_POST['low_stock_limit'] ?? '5'); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="5">
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-card rounded-2xl p-8 space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div id="specifications_container" class="space-y-3">
                            <!-- JS will populate row templates, but we render one default row -->
                ...
                <div class="glass-card rounded-2xl p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="number" name="warranty_months" value="<?php echo htmlspecialchars($_POST['warranty_months'] ?? ''); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="e.g. 12">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-zinc-400">Weight (Grams)</label>
                            <input type="number" name="weight_grams" value="<?php echo htmlspecialchars($_POST['weight_grams'] ?? ''); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="e.g. 221">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-zinc-400">Dimensions</label>
                            <input type="text" name="dimensions" value="<?php echo htmlspecialchars($_POST['dimensions'] ?? ''); ?>" class="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-medium text-white focus:outline-none focus:border-zinc-500 focus:ring-0" placeholder="e.g. 159.9 x 76.7 x 8.3 mm">
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-8 pt-4">
                        <div class="flex items-center gap-3">
                            <select name="status" class="bg-zinc-900 border border-zinc-800 rounded-lg py-2.5 px-4 text-xs font-bold uppercase tracking-widest text-zinc-300 focus:outline-none cursor-pointer">
                                <option value="draft" <?php echo (($_POST['status'] ?? 'draft') === 'draft') ? 'selected' : ''; ?>>Save as Draft</option>
                                <option value="published" <?php echo (($_POST['status'] ?? 'draft') === 'published') ? 'selected' : ''; ?>>Publish Immediately</option>
                            </select>
                        </div>
                    </div>
                </div>
